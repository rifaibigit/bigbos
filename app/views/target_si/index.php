    ...
    <section class="content">
      ...
      <div class="card">
        <div class="card-body">
          <div class="row">
            <div class="col-sm-6">
              <form action="<?= base_url; ?>/TargetSI/importExcel" enctype="multipart/form-data" method="POST" role="form">
                <div class="row">
                  <div class="col-xs-5">
                    <div class="input-group mb-3">
                      ...
                      <div style="margin-left: 5px;">
                        <a href="<?= base_url; ?>/app/upload/Format Upload Target Selling In.xlsx" class="btn float-right btn btn-outline-success"><i class = "fa fa-download"></i></a>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="col-sm-6" style="display: flex; justify-content: flex-end">
                <form action="<?= base_url; ?>/TargetSI" enctype="multipart/form-data" method="POST" role="form">
                  <?php
                      if(isset($data))
                      {
                          $month = $data['by_month'];
                  ...
                  <div class="row">
                    <div class="input-group mb-3">
                      ...
                      <div style="margin-left : 5px;">
                          ...
                          <a class="btn btn-outline-secondary" href="<?= base_url; ?>/TargetSI">Reset</a>
                      </div>
                    </div>
                  </div>
                </form>
            ...
            <table id="target_so" class="table table-bordered nowrap" style="font-size:80%; border: 1px solid black; width: 100%;">
              <thead>
                  <tr>
                      ...
                      <th class="text-center" style="vertical-align: middle;">Month</th>
                      <th class="text-center" style="vertical-align: middle;">Principal</th>
                      <th class="text-center" style="vertical-align: middle;">Area</th>
                      <th class="text-center" style="vertical-align: middle;">Kode Item</th>
              ...
              <tbody>
                  <?php $no=1; ?>
                  <?php foreach ($data['targetSI'] as $row) :?>
                  <tr>
    ...
    excel_file.addEventListener('change', (event) => {
        ...
        reader.onload = function(event){
            ...
            if(sheet_data.length > 1)
            {
              ...
              // if(sheet_data[row][1].toUpperCase() != "TAHUN" || sheet_data[row][2].toUpperCase() != "BULAN" || sheet_data[row][3].toUpperCase() != "PRINCIPAL" || sheet_data[row][4].toUpperCase() != "AREA" || sheet_data[row][5].toUpperCase() != "KODE_ITEM" || sheet_data[row][6].toUpperCase() != "SKU" || sheet_data[row][7].toUpperCase() != "UNIT" || sheet_data[row][8].toUpperCase() != "QTY" || sheet_data[row][9].toUpperCase() != "VALUE")
              // {
              ...
              {
                json.push({'NO': sheet_data[row][0], 'TAHUN': sheet_data[row][1], 'BULAN': sheet_data[row][2], 'PRINCIPAL': sheet_data[row][3], 'AREA': sheet_data[row][4], 'KODE_ITEM': sheet_data[row][5], 'SKU': sheet_data[row][6], 'UNIT': sheet_data[row][7], 'QTY': sheet_data[row][8].replace(/,/g, '').trim(), 'VALUE': sheet_data[row][9].replace(/,/g, '').trim()});
              }
  ...
  <script>
    $(document).ready(function(){
      $('#save').click(function(){ 
        ...
        if(json == '')
        {
          Swal.fire('Error', 'File Excel belum diinput!! Atau Mungkin tidak sesuai dengan template. Harap download ulang template','error');
          return false;
        }
        else
        {
          Swal.fire({
            ...
            if (result.value) {
              $.ajax({
                type: 'POST',
                url:  '<?= base_url; ?>/TargetSI/importExcel2',
                data: {json:json},
                beforeSend: function() {
                  Swal.fire({
                    title: 'Uploading...',
                ...
                success:function(response){
                  Swal.fire({
                      title: "Data Target telah tersimpan.",
                      text: response,
                      type: "success"
                  }).then(function() {
                      window.location = "<?= base_url; ?>/TargetSI";
                  });
                },
