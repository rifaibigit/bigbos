    ...
    <section class="content">
      ...
      <div class="card">
        <div class="card-body">
          <div class="row">
            <div class="col-sm-6">
              <form action="<?= base_url; ?>/TargetSL/importExcel" enctype="multipart/form-data" method="POST" role="form">
                <div class="row">
                  <div class="col-xs-5">
                    <div class="input-group mb-3">
                      ...
                      <div style="margin-left: 5px;">
                        <a href="<?= base_url; ?>/app/upload/Format Upload Target Salesman.xlsx" class="btn float-right btn btn-outline-success"><i class = "fa fa-download"></i></a>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="col-sm-6" style="display: flex; justify-content: flex-end">
                <form action="<?= base_url; ?>/TargetSL" enctype="multipart/form-data" method="POST" role="form">
                  <?php
                      if(isset($data))
                      {
                          $month = $data['by_month'];
                  ...
                  <div class="row">
                    <div class="input-group mb-3">
                      ...
                      <div style="margin-left : 5px;">
                          ...
                          <a class="btn btn-outline-secondary" href="<?= base_url; ?>/TargetSL">Reset</a>
                      </div>
                    </div>
                  </div>
                </form>
          ...
          <div class="table-responsive-xs text-small">
            <table id="target_so" class="table table-bordered nowrap" style="font-size:80%; border: 1px solid black; width: 100%;">
              ...
              <tbody>
                  <?php $no=1; ?>
                  <?php foreach ($data['TargetSL'] as $row) :?>
                  <tr>
    ...
    excel_file.addEventListener('change', (event) => {
        ...
        reader.onload = function(event){
            ...
            if(sheet_data.length > 1)
            {
              ...
              if(sheet_data[1][1].toUpperCase() != "TAHUN" || sheet_data[1][2].toUpperCase() != "BULAN" || sheet_data[1][3].toUpperCase() != "PRINCIPAL" || sheet_data[1][4].toUpperCase() != "AREA" || sheet_data[1][5].toUpperCase() != "SALESMAN" || sheet_data[1][6].toUpperCase() != "QTY" || sheet_data[1][7].toUpperCase() != "VALUE")
              {
              ...
              {
                json.push({'NO': sheet_data[row][0], 'TAHUN': sheet_data[row][1], 'BULAN': sheet_data[row][2], 'PRINCIPAL': sheet_data[row][3], 'AREA': sheet_data[row][4], 'SALESMAN': sheet_data[row][5], 'QTY': sheet_data[row][6], 'VALUE': sheet_data[row][7]});
              }
        ...
        else
        {
          Swal.fire({
            ...
            if (result.value) {
              $.ajax({
                type: 'POST',
                url:  '<?= base_url; ?>/TargetSL/importExcel2',
                data: {json:json},
                beforeSend: function() {
                  Swal.fire({
                    title: 'Uploading...',
                ...
                success:function(response){
                  Swal.fire({
                      title: "Data Target Salesman telah tersimpan.",
                      text: response,
                      type: "success"
                  }).then(function() {
                      window.location = "<?= base_url; ?>/TargetSL";
                  });
                },
