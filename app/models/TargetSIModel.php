...
class TargetSIModel {
	...
	private $table = 'sellingin_target';
	private $db;
	...
	public function getAllTarget()
	{
		...
		{
			$query = "SELECT b.id, a.tahun, a.bulan, a.principal, a.area, a.item_code, b.item_name, b.unit, a.qty, a.value FROM " . $this->table . " a left join sku b on a.item_code = b.item_code WHERE a.tahun = '" . $year . "' ORDER BY tahun, bulan, area, id";
		}
		else
		{
			$query = "SELECT b.id, a.tahun, a.bulan, a.principal, a.area, a.item_code, b.item_name, b.unit, a.qty, a.value FROM " . $this->table . " a left join sku b on a.item_code = b.item_code WHERE a.tahun = '" . $year . "' and CONVERT(a.bulan, SIGNED INTEGER) = '" . (int)$month . "' ORDER BY tahun, bulan, area, id";
		}
    ...
    public function getDuplicateTargetSI($data)
	{
	...
	public function uploadTargetSI($data)
	{
		$error = "";
		$update_count = 0;
		$save_count = 0;
		...
		try
		{
			...
			foreach($data as $row){
				...
				$exist['target_si'] = $this->getDuplicateTargetSI($row);
				
				if($exist['target_si']['tahun'] == $row['tahun'] and $exist['target_si']['bulan'] == $row['bulan'] and $exist['target_si']['area'] == $row['area'] and $exist['target_si']['item_code'] == $row['item_code'])
				{
