...
class StokModel {
	...
	public function getAllStok()
	{
		...
		{
			$query = "SELECT b.id, a.tahun, a.bulan, a.principal, a.area, a.item_code, b.item_name, b.unit, a.qty, a.value FROM " . $this->table . " a inner join sku b on a.item_code = b.item_code WHERE a.tahun = '" . $year . "' and a.bulan = '" . (int)$month . "' ORDER BY tahun, bulan, area, id";
		}
	...
	{
		$this->db->opendb();
		$query = "INSERT INTO stok (tahun, bulan, principal, area, item_code, qty, value, create_by, create_date) VALUES(:tahun, :bulan, :principal, :area, :item_code, :qty, :value, :create_by, now())";
		$this->db->query($query);
		...
		//Flasher::setMessage('Berhasil',$data['tahun'].",".$data['bulan'].",".$data['principal'].",".$data['area'].",".$data['item_code'].",".$data['qty'].",".$data['value'],'success');
	...
	{
		$query = "UPDATE " . $this->table . " SET qty=:qty, value=:value, edit_by=:edit_by, edit_date=now() WHERE tahun=:tahun and bulan=:bulan and item_code=:item_code and area=:area";
		$this->db->opendb();
		$this->db->query($query);
		$this->db->bind('qty', $data['qty']);
        $this->db->bind('value', $data['value']);
    ...
    public function getDuplicateStok($data)
	{
		$query = "SELECT * FROM " . $this->table . " WHERE tahun=:tahun and bulan=:bulan and principal=:principal and area=:area and item_code=:item_code";
		//$this->db->opendb();
		$this->db->query($query);
		$this->db->bind('tahun', $data['tahun']);
        $this->db->bind('bulan', $data['bulan']);
	...
	public function uploadStok($data)
	{
		$error = "";
		$update_count = 0;
		$save_count = 0;
		...
		try
		{
			...
			foreach($data as $row){
				...
				if($row['item_code'] == "#N/A" or $row['item_code'] == "")
                {
                    ...
                    $error = "Error pada data TAHUN : '".$row['item_code']."' di baris : '".$baris."'";
					throw new Exception($error);
				...
				$exist['stok'] = $this->getDuplicateStok($row);
				
				if($exist['stok']['tahun'] == $row['tahun'] and $exist['stok']['bulan'] == $row['bulan'] and $exist['stok']['area'] == $row['area'] and $exist['stok']['item_code'] == $row['item_code'])
				{
